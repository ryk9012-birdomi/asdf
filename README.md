...
- **3D 무대:** 화면 위쪽에 동굴 무대가 있고, 옆에서 본 시점으로 일행(왼쪽)과 적(오른쪽)이 마주 섭니다. 무대는 횃불, 폐허 기둥, 불씨로 꾸몄습니다.
  - 근접 공격을 하면 대상 앞까지 달려가 베고 돌아옵니다.
  - 주문과 화살은 발사체가 날아갑니다.
  - 맞은 쪽은 휘청이고, 빗나가면 뒤로 피하고, 쓰러지면 쓰러진 채로 남습니다. 전투가 끝나면 이긴 쪽이 환호합니다.
  - 주사위와 피해 숫자는 캐릭터 머리 위에, 공격이 닿는 순간에 맞춰 뜹니다.
  - 발밑 고리가 현재 행동자(금색)와 지정 가능한 대상(초록색)을 표시하며, **3D 캐릭터를 직접 클릭해도** 대상을 지정할 수 있습니다.
- 3D 모델은 [KayKit Adventurers Character Pack 1.0](https://kaylousberg.com)(Kay Lousberg, **CC0**)을 씁니다.
  - 기사 → 알데릭, 도적 → 시엔, 마법사 → 엘로웬
  - 적은 같은 모델의 크기와 색조를 바꿨습니다: 야만전사 → 고블린 약탈자·홉고블린 대장, 두건 도적 → 궁수, 마법사 → 잿불 사제
  - 파일은 `godot/assets/models/kaykit/`에 있습니다.
- 아래쪽 카드는 정보창입니다. HP, 보호막, 기력, 적의 다음 행동과 대상별 확률을 보여 줍니다.
