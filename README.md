...
- **2D 전장:** 횃불이 타는 고갯길 동굴을 옆에서 본 화면입니다. 일행은 왼쪽, 적은 오른쪽에 서며, 양쪽 모두 전열이 가운데 가장 가깝게 섭니다.
  ...
  - **알데릭**은 부위별 SVG 그림(판금·타바드·망토·투구 깃털·장검·연 모양 방패)을 관절로 이어 붙인 컷아웃 인형입니다. 걸을 때 다리가 움직이고, 망토와 깃털은 한 박자 늦게 따라 흔들립니다. 휘두를 때는 기합, 맞으면 찡그림, 쓰러지면 눈을 감는 표정이 있고, 가끔 눈을 깜박입니다. 기도할 때는 검에, 방패를 올릴 때는 방패에 빛이 모입니다.
  - **기사 견본 12종:** `scenes/dev/KnightGallery.tscn`을 열고 F6을 누르면 알데릭, 성전 기사, 흑기사, 황금 성기사, 달빛 기사, 진홍 기사, 숲의 파수꾼, 왕실 근위대, 늙은 용병, 견습 기사, 망자의 기사, 태양의 기사단장이 같은 뼈대로 움직입니다. 위쪽 버튼으로 대기·베기·기도·방패·피격·승리·쓰러짐을 한꺼번에 재생할 수 있습니다.
  - 견본은 `tools/generate_knight_art.py`의 `VARIANTS`에서 강철 색·장식 금속·겉옷과 문장·망토·투구(열린 투구/대투구/곡면 투구/날개/뿔/왕관/맨머리)·얼굴(피부·수염·눈)·무기(장검/단검/대검/철퇴/전투 망치/도끼/창)·방패(연 모양/삼각/원형/탑/없음)를 골라 만듭니다. 항목을 고치거나 추가한 뒤 스크립트를 실행하면 SVG가 다시 만들어집니다.
  - 나머지 6종은 아직 코드로 직접 그린 그림입니다(`FighterArt.gd`). 알데릭과 같은 방식으로 차례로 바꿀 예정입니다.
...
├─ art/        heroes/paladin — 알데릭 컷아웃 인형의 부위별 SVG(3배 해상도로 가져옴)
│              knights/<id> — 기사 견본 12종 (tools/generate_knight_art.py로 생성)
