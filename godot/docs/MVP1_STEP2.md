...
로그·위저드·팔라딘이 고블린 약탈자 3명과 싸우는 한 번의 전투를 끝까지 플레이합니다. Speed 기반 행동 순서, 기본 공격과 스킬, 스킬 선택 → 대상 선택, 2d6 판정(명중·스침·치명타), 보호막, 기력·쿨다운, 사망자 제외, 승리·패배, 재시작을 구현했습니다. 적은 공격 → 보호막 → 화염병 패턴을 반복하고 다음 행동을 표시합니다.
...
| `res://scripts/battle/DamageCalculator.gd` | 2d6 판정, 정확한 확률(36가지 경우) 계산, 로그 문구 |
...
**피해:** 타격마다 6면체 주사위 2개를 굴려 `합 + 공격자 hit_bonus − 대상 defense`로 판정합니다. 6 이하는 빗나감, 7~9는 스침(피해 절반, 올림), 10 이상은 명중입니다. 보정 전 주사위 합이 공격자의 `crit_threshold`(7~12, 기본 12) 이상이면 보정과 관계없이 치명타로 피해가 2배입니다. 기본량은 `round(공격력 × 배율) + 고정값`이며 방어는 피해를 줄이지 않고 명중 판정에만 쓰입니다. `auto_hit` 스킬은 굴림 없이 명중합니다. TRUE_DAMAGE는 대상 방어를 무시하지만 보호막에는 흡수됩니다. PHYSICAL/ARCANE/FIRE/RADIANT는 현재 연출만 다릅니다. UI는 36가지 경우를 모두 세어 정확한 확률을 대상 카드와 적 예고에 표시합니다. HP 규모는 레벨 1~2 유닛 10 이하, 레벨 5 이상 정예·보스 20 이상입니다. 계산된 피해는 CharacterUnit의 보호막 → HP 순서로 적용합니다.
