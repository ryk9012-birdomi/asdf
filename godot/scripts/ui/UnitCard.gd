...
func _ready() -> void:
	...
	frame.bg_color = Color("111d30")
	frame.border_color = accent.darkened(0.55)
	frame.set_border_width_all(1)
	frame.set_corner_radius_all(12)
	add_theme_stylebox_override("panel", frame)
	add_label(["01   /   FRONT", "02   /   MIDDLE", "03   /   BACK"][formation_slot], 13, accent)
	...
	add_label(character_data.character_name, 26)
	add_label(character_data.class_name_label, 15, accent)
	var description := add_label(character_data.description, 13, Color("a4b4ca"))
	...
	add_label("명중 %d%%   회피 %d%%   치명 %d%%" % [roundi(character_data.accuracy * 100), roundi(character_data.evasion * 100), roundi(character_data.critical_chance * 100)], 12, Color("a4b4ca"))
	health_label = add_label("", 14)
	health_bar = add_bar(accent, 10)
	resource_label = add_label("", 13, Color("b9c9df"))
	energy_bar = add_bar(Color("7597f4"), 5)
	state_label = add_label("", 12, accent)
	content.add_child(HSeparator.new())
	add_label("LOADOUT  /  스킬 정의", 12, Color("a4b4ca"))
	for skill in character_data.skills:
		var entry := add_label("%s   ·   EN %d / CD %d" % [skill.skill_name, skill.energy_cost, skill.cooldown], 14)
	...
	content.add_child(HSeparator.new())
	add_label("상태 테스트", 12, Color("a4b4ca"))
	var actions := GridContainer.new()
	actions.columns = 2
	content.add_child(actions)
	var labels := ["피해 30", "회복 25", "실드 +20", "에너지 −2", "충전 +2", "치명상"]
...
func refresh() -> void:
	...
	resource_label.text = "실드 %d     ·     에너지 %d / %d" % [unit.current_shield, unit.current_energy, unit.max_energy]
	energy_bar.max_value = maxi(1, unit.max_energy)
	energy_bar.value = unit.current_energy
	state_label.text = "●  작전 가능" if unit.is_alive() else "×  전투 불능 · 전체 초기화로 복구"
	portrait.modulate = Color.WHITE if unit.is_alive() else Color("4c5360")
	for button in action_buttons:
		button.disabled = not unit.is_alive()


func add_label(value: String, font_size: int, color: Color = Color("e8effa")) -> Label:
	var label := Label.new()
	label.text = value
...
func add_bar(color: Color, height: int) -> ProgressBar:
	...
	background.bg_color = Color("26364b")
