...
func run_tests() -> void:
	...
	check(first.receive_damage(3) == 2, "Shield absorbs the first point of damage")
	check(first.current_hp == 6 and first.current_shield == 0, "Damage crosses shield into health")
	check(second.current_hp == 8 and second.current_shield == 1, "Second instance remains independent")
	check(definition.max_hp == 8 and definition.starting_shield == 1, "Shared resource remains unchanged")
	...
	check(first.heal(999) == 2 and first.current_hp == 8, "Healing is capped at maximum")
	first.add_shield(4)
	check(first.receive_damage(1) == 0 and first.current_shield == 3, "Shield-only damage preserves HP")
	check(first.receive_damage(2, true) == 2 and first.current_shield == 3, "Explicit shield bypass preserves shield")
	...
	check(first.current_shield == 1 and first.current_energy == 5, "Reset restores original resources")
	first.receive_damage(9999)
