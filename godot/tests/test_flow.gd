...
func run_tests() -> void:
	...
	check(menu.training_button.text == "훈련장" and menu.quit_button.text == "종료", "Training grounds and quit entries exist")
	...
	menu.training_button.pressed.emit()
	await settle()
	check(current_scene.name == "CharacterLab", "Menu opens the training grounds")
	current_scene.get_node("%ResetButton").get_parent().get_node("MenuButton").pressed.emit()
	await settle()
	check(current_scene.name == "MainMenu", "Training grounds return to the main menu")
